...
class ARRCalculatorGUI:
    ...
    def create_widgets(self):
        ...
        # Copyright notice
        copyright_text = "© 2024 Nicolo Zavarise - Tesselate"
...
Data Sources:
  • CSV File: Upload your own rate data file
  • Bank of England: Automatically fetch SONIA rates from the official website
    (requires internet connection, only available for SONIA calculations)

CSV File Format:
Your CSV file should have:
  • Column 1: Date in YYYY-MM-DD format
  • Column 2: Daily rate (can be percent like 5.12 or decimal like 0.0512)
  • Optional header row (will be auto-detected)
...
Key Concepts:
  • Lookback: Number of business days to look back for rate observation
  • Margin: Additional spread added to the RFR (not compounded)
  • CAS: Credit Adjustment Spread (not compounded)
  • Day Count: ACT/365F for SONIA, ACT/360 for SOFR

Common Issues:
  • Ensure CSV has sufficient historical data for lookback period
  • Check that dates are in correct format
  • Verify rates are reasonable (typically 0-20% range)
  • Make sure end date is after start date

Input Guidelines:
  • Principal: Enter the loan amount (e.g., 1000000.00)
  • Dates: Use YYYY-MM-DD format (e.g., 2024-01-01)
  • Rates: Enter as percentages (e.g., 2.00 for 2%)
  • Lookback: Typically 5 business days for SONIA/SOFR
        """
...
def compute_interest_compounded_in_arrears(
    ...
    while current_date < end:
        ...
        # Formula: (1 + (r * days_applied) / N) — matches Compounded-in-Arrears using n_i calendar days per business day
